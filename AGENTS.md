...
Run `bin/mastermenu-clean --keep-days N` to prune old timestamped workdirs. Tag CLI-friendly tools with `cli` in `app.yaml` so wrappers stay in sync. Store sensitive data outside the repo and reference via environment variables. When retiring a tool, remove its folder, wrapper, and `menu_config.yaml` references, then run `scripts/doctor.sh` to confirm a clean slate.
